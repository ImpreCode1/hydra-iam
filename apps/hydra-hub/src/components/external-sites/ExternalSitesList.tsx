...
import { EditExternalSiteModal } from "./EditExternalSiteModal";
...
export function ExternalSitesList({ refreshKey }: ExternalSitesListProps) {
  ...
  async function toggleStatus(site: ExternalSite) {
    try {
      ...
      await updateExternalSite(site.id, { isActive: updated });
      setSites((prev) =>
  ...
  function handleUpdate(updated: ExternalSite) {
    setSites((prev) =>
    ...
    <div className="space-y-4">
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 font-mono truncate" title={site.url}>
                  {site.url}
                </p>
                <span
      ...
      {!loading && filteredSites.length === 0 && (
        <div className="text-center py-12 text-slate-500">
          {search ? "No se encontraron sitios" : "No hay sitios externos registrados"}
        </div>
      )}

      {selectedSite && (
        <EditExternalSiteModal
          site={selectedSite}
